    ...
    useEffect(() => {
        if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                if (uv !== null) {
                    // Check whenever dependencies change, but we need the UV value.
                    // Storing UV in state would be better, but for MVP we just check here or in a separate effect.
                    // Let's store UV in a ref or state if we want to re-evaluate on ingredient change.
                    // For now, let's just save it to a window variable or similar for the other effect? 
                    // Actually, let's add a separate state for 'uvIndex'.
                    setUvData(uv);
                }
            });
        }
